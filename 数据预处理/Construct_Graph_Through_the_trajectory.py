...
def construct_adjacency_list(Last_Centroid_Points,Current_Centroid_Points,Last_Index,Current_Index,graph):
    ...
    if not flag1:
        ...
        # print("增加权重")
def init_graph_and_Statistics_Intersection_Corresponding_To_Vehicel(date,strtime):
    graph = Graph()
    #合并出的中心拐点的路径，并加载所有中心拐点
    # Path_Centroid = f"../DT_Clustering-main/Class_All_Centroid_Points/2019-10-17-19-Filter_Inflection_Point.txt"
    # Path_Centroid1 = "../../my_result/traffic_wuhan_data/result/2019-10-" + str(
    #     date) + "/sort_time/point_result/merge_point/Clusters/Class_All_Centroid_Points/Class_All_Centroid_Points.txt"
    Path_Centroid1 = "Filter_Inflection_All_Centroid_Points/Filter_Inflection_All_Centroid_Points.txt"
    Centroid_Points1 = pd.read_csv(Path_Centroid1, delimiter=',')
    Graph_name = "2019-10-"+str(date)+"_day_Construct_result_"
    # Graph_name = "2019-10-17---19_day_Construct_result_"
    Centroid_Points_Correspond_Index = []
    # 初始化图，并将图中每一个点对应的坐标保存到列表中
    for k in range(len(Centroid_Points1)):
        current_point = Centroid_Points1.iloc[k:k + 1].reset_index(drop=True).to_numpy()
...
def main():
    ...
    # parser.add_argument('--vehicel_len', type=int, default="18353", choices=[2,11154, 11180, 10558,18353,17940,17148])
    ...
    day = [17,18,19]
    list11 = [18353,17940,17148]#18353,17940,17148
    list_i = 0
    ...
    for date in day:
        for i in range(0,24):
            ...
            graph, Statistics_Intersection_Corresponding_To_Vehicel, Graph_name, Centroid_Points_Correspond_Index = init_graph_and_Statistics_Intersection_Corresponding_To_Vehicel(date,strtime)
            if i < 9:
            ...
            for k in range(1, list11[list_i]):  # 11153
                vehicel_flag = 0
                # print("date=", date)
                Path1 = "../../my_result/traffic_wuhan_data/result/2019-10-" + str(date) + "/sort_time/vehicel" + str(
                    k) + "_result.txt"
                # Path = f"../traffic_nanjing_data/result/2019-10-" + str(date) + "/sort_time/vehicel" + str(
                #     k) + "_result.txt"
                # 加载数据
                vehicel = pd.read_csv(Path1, delimiter=',')
            ...
            #保存每个时间段（一个小时）的保存文件
            # folder_path1 = "graph_file/"+ str(date) + "/" + strtime +"/" # 将这里替换为实际想要检查和创建的文件夹路径
            # folder_path1 = "graph_file/all/" + strtime +"/"
            # if not os.path.exists(folder_path1):
            #     os.makedirs(folder_path1)
            #     print(f"文件夹 {folder_path1} 不存在，已成功创建。")
            # else:
            #     print(f"文件夹 {folder_path1} 已存在。")
            # graph.to_json(folder_path1+Graph_name + strtime + ".json")
            # # print(start_datetime,"-",end_datetime,"的图文件保存完成")
            #
            # # 将统计的路口车流量对应的字典保存到JSON文件中
            # with open(folder_path1+'Statistics_Intersection_Corresponding_To_Vehicel_Filter_result' + strtime + '.json', 'w',
            #           encoding='utf-8') as f:
    ...
    #保存每个时间段（一个小时）的保存文件
    folder_path1 = "graph_file/"+ str(date) + "/" + strtime +"/" # 将这里替换为实际想要检查和创建的文件夹路径
    folder_path1 = "graph_file/all/"
    if not os.path.exists(folder_path1):
        os.makedirs(folder_path1)
        print(f"文件夹 {folder_path1} 不存在，已成功创建。")
    else:
        print(f"文件夹 {folder_path1} 已存在。")
    graph.to_json(folder_path1 + "day_Construct_result.json")
    print(start_datetime,"-",end_datetime,"的图文件保存完成")

    # 将统计的路口车流量对应的字典保存到JSON文件中
    with open(folder_path1 + 'Statistics_Intersection_Corresponding_To_Vehicel_Filter_result' + '.json', 'w',
              encoding='utf-8') as f:
        json.dump(Statistics_Intersection_Corresponding_To_Vehicel, f, ensure_ascii=False, indent=4)
    print(start_datetime,"-",end_datetime,"对应的点流量统计数据完成")

if __name__ == "__main__":
    print("----------------------------------7、根据上述步骤筛选出的拐点-------------------------------------")
