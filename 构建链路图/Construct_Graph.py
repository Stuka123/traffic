...
def isExistRegion(current_vehicel,Current_Centroid_Points):
    ...
    # current_vehicel1 = [118.80379263185469, 32.55228534491289]
    # Current_Centroid_Points1 = [118.80373056936492, 32.55228688505415]
    # if str(Centroid_Points_lng) == str(Current_Centroid_Points1[0]) and str(Centroid_Points_lat) == str(Current_Centroid_Points1[1]):
    #     if is_in_region == True:
    # print("Sign = ",Sign,":","is_in_region = ",is_in_region)

    # print("is_in_region=", is_in_region)
...
def Record_Vehcle(Last_Index,Current_Index,vehicel_index,Statistics_Intersection_Corresponding_To_Vehicel):
    ...
    Value = vehicel_index[0],vehicel_index[1],Current_Index
...
def main():
    ...
    Graph_name = "2019-10-17-19_day_17-final_test1"
    ...
    for date in day:
        if date == 17:
            ...
            end_time = '2019-10-17 23:59:59'
            start_datetime = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
            end_datetime = datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')
        if date == 18:
            start_time = '2019-10-18 00:00:00'
            end_time = '2019-10-18 23:59:59'
            start_datetime = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
            end_datetime = datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')
        if date == 19:
            start_time = '2019-10-19 00:00:00'
            end_time = '2019-10-19 23:59:59'
            start_datetime = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
            end_datetime = datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')
        print("运行日期：",date, start_time,"-",end_time,list11[list_i])
        for k in range(1, list11[list_i]):  # 11153
            ...
            print("长度=", len(filtered_vehicel))
            print("第%s辆车" % k)
            # 遍历车辆的所有轨迹点
            for i in range(len(filtered_vehicel)):
                Current_vehicel_points = filtered_vehicel.iloc[i:i + 1]
                original_indices = Current_vehicel_points.index.tolist()[0] + 2
                # print("original_indices=",original_indices[0]+2)
                # print("这是第%s辆车的第%s个轨迹点" %(k,i))
                # print("Current_vehicel_points=",Current_vehicel_points)
                # 遍历所有的质心
                for j in range(len(Centroid_Points_Correspond_Index)):
                    ...
                    if Judge:
                        # print("original_indices=", original_indices, "flag=", flag, "  i=", i,"j=",j,
                        #       (Last_Index != Current_Index),(Last_Index , Current_Index))
                        # 记录通过拐点的车辆轨迹及其方向
                        # vehicel_index = k, original_indices
                        # Record_Vehcle(Current_Index, vehicel_index,Statistics_Intersection_Corresponding_To_Vehicel)
                        if flag == 0 or vehicel_flag == 0:
                            ...
                            if Last_Index != Current_Index:
                                ...
                                Record_Vehcle(Last_Index, Current_Index, Last_Vehicle_Index,
                                              Statistics_Intersection_Corresponding_To_Vehicel)
                                # print("Last_Vehicle_Index = ",Last_Vehicle_Index,"Current_Vehicle_Index = ",(k, original_indices),
                                #       "Last_Index=",Last_Index,"->","Current_Index=",Current_Index)
                                vehicel_index = k, original_indices
                                Last_Vehicle_Index = vehicel_index

                                # print("\n前：","Last_Index=",Last_Index , "Current_Index=",Current_Index)
                                # 记录上一次保存的节点
                                Last_Centroid_Points = Current_Centroid_Points
                                Last_Index = Current_Index
                                # print("后：","Last_Index=",Last_Index,"Current_Index=", Current_Index,"\n")
                                flag += 1
                                break
                            else:
                                Record_Vehcle(Last_Index, Current_Index, Last_Vehicle_Index,
    ...
    # result_save_path = f"../DT_Clustering-main/Class_All_Centroid_Points/"
    # with open(result_save_path + "result.txt", "w") as file:
    #     file.write("lng,lat" + "\n")  # 在每个元素后添加换行符
    # with open(result_save_path + "result.txt", "a") as file:
    #     for i in range(len(result)):
    #         r = str(result[i]).replace("(", "").replace(")", "")
    # print(graph)
    ...
    # 将字典保存到JSON文件中
    with open('Statistics_Intersection_Corresponding_To_Vehicel1.json', 'w', encoding='utf-8') as f:
    ...
    # 读取并加载JSON文件中的数据
    with open('Statistics_Intersection_Corresponding_To_Vehicel1.json', 'r', encoding='utf-8') as f:
