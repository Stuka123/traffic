...
def generate_point_Matrix(result_flow_graph):
    # 通过整个图来构建文件
    ...
    for vertex in all_result:
        ...
        for edge in all_result[vertex]:
            flow.append(edge[1])
        if vertex not in result_flow_graph:
            result_flow_graph[vertex] = np.sum(flow)
    ...
    # 计算每一个拐点对应的速度
    result_speed_graph = {}
    with open('graph_file/all/Statistics_Intersection_Corresponding_To_Vehicel_Filter_result' + '.json', 'r',
              encoding='utf-8') as f:
        loaded_data = json.load(f)
    for vertex in loaded_data:
        # print(vertex, "->", loaded_data[vertex])
        speed = []
        for edge in loaded_data[vertex]:
            speed.append(edge[1])
        if vertex not in result_speed_graph:
            result_speed_graph[vertex] = np.mean(loaded_data[vertex]) / 100
    ...
    arr = np.random.rand(72, 540, 2)
    print(arr)
    print("---------------------")
    # arr[0][0] = [1,1]
    # print(arr[0],arr[0][0],arr[0][0][0])
    for i in range(72):
        for j in range(540):
            flow = result_flow_graph[str(j)]
            speed = result_speed_graph[str(j)]
            value = [float(flow), float(speed)]
            arr[i][j] = value
...
if __name__ == '__main__':
    ...
    # generate_point_Matrix(result_flow_graph)
    arr = np.random.rand(72, 8780, 2)
    print(arr)
    print("---------------------")
    # arr[0][0] = [1,1]
    # print(arr[0],arr[0][0],arr[0][0][0])
    ...
    # # result_final = {}
    # # result_final["data"] = arr
    # # 使用 np.savez 函数将数组保存为 npz 文件
    # np.savez('PEMS041.npz', data=arr)
    #
